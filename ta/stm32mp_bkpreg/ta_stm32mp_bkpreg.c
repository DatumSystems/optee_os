...
static TEE_Result bkpreg_read(uint32_t pt, TEE_Param params[TEE_NUM_PARAMS])
{
	...
	params_pta[0].value.a = out_start * sizeof(uint32_t);
	params_pta[0].value.b = 0;
	params_pta[1].memref.buffer = out;
	params_pta[1].memref.size = out_size * sizeof(uint32_t);
...
static TEE_Result bkpreg_write(uint32_t pt, TEE_Param params[TEE_NUM_PARAMS])
{
	...
	params_pta[0].value.a = in_start * sizeof(uint32_t);
	params_pta[0].value.b = 0;
	params_pta[1].memref.buffer = in;
	params_pta[1].memref.size = in_size * sizeof(uint32_t);

	res = TEE_InvokeTACommand(pta_session,	TEE_TIMEOUT_INFINITE,
						PTA_BKPREG_WRITE_REG,
						pt, params_pta, NULL);
	return res;
}
...
{
	switch (cmd) {
	case TA_STM32MP_BKPREG_READ:
		return bkpreg_read(pt, params);
	case TA_STM32MP_BKPREG_WRITE:
		return bkpreg_write(pt, params);
	default:
